module Jelly
  module VirtualMachine
    ...
    class ProcessLinks
      ...
      def get_links(pid : UInt64) : Array(UInt64)
        @mutex.synchronize do
          @links[pid].to_a
        end
      end
      ...
      def trap_exit(pid : UInt64, enable : Bool = true)
        @mutex.synchronize do
          if enable
            @trap_exit.add(pid)
            Log.debug { "Process <#{pid}> now trapping exits" }
          else
            @trap_exit.delete(pid)
            Log.debug { "Process <#{pid}> no longer trapping exits" }
          end
        end
      end

      # Check if a process traps exits
      def traps_exit?(pid : UInt64) : Bool
        @mutex.synchronize do
          @trap_exit.includes?(pid)
        end
      end
